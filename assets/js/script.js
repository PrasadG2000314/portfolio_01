...
(function() {
  ...
  emailjs.init({
    publicKey: "XHrRtL4I4og8FGLSs",
  });
})();

sueContactForm = document.getElementById("sue-contact-form");
sueContactFromAlert = document.querySelector(".contact-form-alert");

sueContactForm.addEventListener('submit', function(event) {
    event.preventDefault();
    // these IDs from the previous steps
    emailjs.sendForm('service_1fef1kp', 'template_s16f7sw', ' #sue-contact-form')
    .then(() => {
       // console.log('SUCCESS!');
...
window.addEventListener("scroll", () => {
  ...
  // Add or remove 'active' class to show/hide 'To Top' button based on scroll position
  toTopBtn.classList.toggle("active", window.scrollY > 0);

  // Update the scroll indicator bar
  const scrollIndicatorBar = document.querySelector(".scroll-indicator-bar");

  // Determine the scroll position as a percentage of the total page height
  const pageScroll = document.body.scrollTop || document.documentElement.scrollTop;
  const height = document.documentElement.scrollHeight - document.documentElement.clientHeight;
  const scrollValue = (pageScroll / height) * 100;

  // Adjust the height of the scroll indicator bar
...
const cursorHoverlink = document.querySelectorAll("body a,theme-btn, .sue-main-btn, .portfolio-card, .swiper-btn-next, .swiper-btn-prev, .swiper-pagination-bullet, .service-card, .contact-social-links li, .contact-form, .submit-btn, .menu-show-btn, .menu-show-btn, .menu-hide-btn");
...
// Apply reveal animations to targeted elements
...
ScrollReveal().reveal('.pro-card, .about-buttons .sue-main-btn, .resume-tabs .tab-btn, .portfolio-tab .tab-btn', { delay: 500, origin: 'bottom' });
ScrollReveal().reveal('#resume .section-content', { delay: 700, origin: 'bottom' });
ScrollReveal().reveal('.service-card, .portfolio-card, .contact-item, .contact-social-link li, .footer-menu .menu-item', { delay: 300, origin: 'bottom' });
ScrollReveal().reveal('.service-card, .portfolio-card, .contact-item, .contact-social-links li', { delay: 300 });
ScrollReveal().reveal('.sue-client-swiper, .contact-form-body, .portfolio-card', { delay: 700, origin: 'right' });
